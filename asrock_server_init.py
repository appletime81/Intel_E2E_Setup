...
class App:
    def run(self):
        ...
        # cd root path
        root_path = input(
            f"{bcolors.OKBLUE}Go to {bcolors.ENDC}{bcolors.WARNING}{ROOT_DIR[self.server_host]}{bcolors.ENDC} {bcolors.OKBLUE}? (y/n){bcolors.ENDC}"
        )
        if root_path == "y":
    ...
    def run_rtp_setting(self):
        if self.server_host == "50":
            print(
                f"{bcolors.OKBLUE}Start running {bcolors.ENDC}{bcolors.WARNING}do_ptp.sh{bcolors.ENDC}"
            )
            os.system("./do_ptp.sh")
            print(f"{bcolors.WARNING}Running done !{bcolors.ENDC}")
        elif self.server_host == "25":
            print(
                f"{bcolors.OKBLUE}Start running {bcolors.ENDC}{bcolors.WARNING}./setup_ota_e2e ptp_old{bcolors.ENDC}"
            )
    ...
    def gnbctl_init(self):
        for item in INIT_LIST:
            print(
                f"{bcolors.OKBLUE}Start Init {bcolors.ENDC}{bcolors.WARNING}{item}{bcolors.ENDC}"
            )
            os.system(f"./gnbctl {item} init")
            print(
                f"{bcolors.WARNING}{item}{bcolors.ENDC} {bcolors.OKBLUE}init done !{bcolors.ENDC}"
            )
